function [modVals] = word2vec_pos_intrainter_neg(ucfClass,TopSelectedVids,decVals,V2T,VideoID,TagID,TestOrder,word2vecMat)
    hypo_tags = {};
    hypo_tags(1,:) = {'bowling','','','','','','',''};
...
% %      For rest of the videos check how many tags of it are common   
        for rest_video=1:ovVids
            rest_commonTags = 0;
            rest_ID = VideoID(totalList{rest_video});
            if ip_ID==rest_ID
                continue
            end
            ...
            if isKey(TopSelectedVids,totalList{rest_video})
                if TopSelectedVids(totalList{rest_video}) == 1
